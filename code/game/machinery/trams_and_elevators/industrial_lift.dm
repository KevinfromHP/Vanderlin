...
/obj/structure/industrial_lift/proc/unset_movement_registrations(list/turfs_to_unset)
	var/static/list/registrations = list(COMSIG_ATOM_ENTERED, COMSIG_ATOM_EXITED, COMSIG_ATOM_AFTER_SUCCESSFUL_INITIALIZED_ON, COMSIG_TURF_EXITED, COMSIG_TURF_ENTERED)
...
/obj/structure/industrial_lift/tram/storage
	var/icon/icon_state_full
	var/icon/icon_state_empty
	var/list/stuff_shit = list()
	var/current_capacity = 0
	var/maximum_capacity = 30 //arbitrary maximum amount of weight allowed in the cart before it says fuck off
	var/arbitrary_living_creature_weight = 10 // The arbitrary weight for any thing of a mob and living variety

/obj/structure/industrial_lift/tram/storage/container_resist(mob/living/user)
...
/obj/structure/industrial_lift/tram/storage/dump_contents()
	var/atom/L = drop_location()
...
/obj/structure/industrial_lift/tram/storage/Destroy()
	dump_contents()
	return ..()

/obj/structure/industrial_lift/tram/storage/MouseDrop_T(atom/movable/O, mob/living/user)
...
/obj/structure/industrial_lift/tram/storage/attackby(obj/item/I, mob/user, params)
	if(!user.cmode)
		if(put_in(I, user))
			playsound(loc, 'sound/foley/cartadd.ogg', 100, FALSE, -1)
		return
...
/obj/structure/industrial_lift/tram/storage/attack_hand(mob/living/user)
	. = ..()
	if(.)
		return
	if(user.cmode)
...
/obj/structure/industrial_lift/tram/storage/proc/put_in(atom/movable/O, mob/user)
...
/obj/structure/industrial_lift/tram/storage/proc/take_contents()
	var/atom/L = drop_location()
	for(var/atom/movable/AM in L)
		if(AM != src && put_in(AM)) // limit reached
			break

/obj/structure/industrial_lift/tram/storage/update_icon()
	. = ..()
	cut_overlays()
...
/obj/structure/industrial_lift/tram/storage/attack_right(mob/user)
	. = ..()
	if(.)
		return
	user.changeNext_move(CLICK_CD_MELEE)
...
/obj/structure/industrial_lift/tram/storage/proc/insertion_allowed(atom/movable/AM)
	if(ismob(AM))
